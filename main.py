...
MAX_LOG_LINES = 2000
JOBS: Dict[str, Dict[str, Any]] = {}
EXEC = ThreadPoolExecutor(max_workers=1)  # avoid OOM on Render
...
def scan_dir(scan_id: str) -> str:
    return os.path.join(BASE_DIR, f"scan_id_{scan_id}")

def meta_file(scan_id: str) -> str:
    return os.path.join(scan_dir(scan_id), "meta.json")

def log_file(scan_id: str) -> str:
...
def write_meta(scan_id: str, data: dict):
    ...
    with open(meta_file(scan_id), "w", encoding="utf-8") as f:
...
def append_log(scan_id: str, line: str):
    d = scan_dir(scan_id)
    os.makedirs(d, exist_ok=True)
    with open(log_file(scan_id), "a", encoding="utf-8") as f:
...
# =========================================================
# JADX EXECUTION (LIVE LOG STREAM)
# =========================================================
def run_jadx(scan_id: str, output_dir: str, apk_path: str, timeout_sec: int = 3600):
    cmd = [
        "jadx",
        "--threads-count", "1",
        "--verbose",
    ...
    push_log(scan_id, "Starting JADX")
    push_log(scan_id, "CMD: " + " ".join(cmd))
    ...
    if proc.returncode != 0:
        raise RuntimeError(f"JADX failed (exit {proc.returncode})")

    push_log(scan_id, "JADX completed successfully")
...
def worker(scan_id: str, apk_url: str):
    try:
        ...
        apk_path = os.path.join(out_dir, "app.apk")

        JOBS[scan_id]["status"] = "downloading"
        write_meta(scan_id, {"status": "downloading", "apk_url": apk_url, "output_dir": out_dir})
        push_log(scan_id, f"Downloading APK: {apk_url}")
        ...
        push_log(scan_id, f"Download complete: {size/1024/1024:.2f} MB")

        JOBS[scan_id]["status"] = "decompiling"
        write_meta(scan_id, {"status": "decompiling", "apk_url": apk_url, "output_dir": out_dir})
        push_log(scan_id, "Running JADX")

        run_jadx(scan_id, out_dir, apk_path)
        ...
        push_log(scan_id, "SCAN COMPLETED")
...
# =========================================================
@app.post("/decompile")
def decompile(payload: DecompileFromUrlReq):
    scan_id = payload.scan_id or str(ObjectId())

    JOBS[scan_id] = {"status": "queued", "apk_url": payload.apk_url}
    init_job_logs(scan_id)

    os.makedirs(scan_dir(scan_id), exist_ok=True)
    write_meta(scan_id, {"status": "queued", "apk_url": payload.apk_url})
...
@app.get("/logs/{scan_id}")
def logs(scan_id: str, tail: int = 400):
    lf = log_file(scan_id)
...
def browse(scan_id: str, path: str = ""):
    ...
    if os.path.isfile(target):
        return {"type": "file", "name": os.path.basename(target)}
...
def read_file(scan_id: str, path: str, max_kb: int = 256):
    ...
    size = os.path.getsize(target)
    with open(target, "rb") as f:
        data = f.read(min(size, max_kb * 1024))

    return {
        "path": path,
        "size_bytes": size,
        "truncated": size > max_kb * 1024,
...
def scans():
    return {
        "scan_ids": [
            ...
            if d.startswith("scan_id_") and os.path.isdir(os.path.join(BASE_DIR, d))
        ]
    }
